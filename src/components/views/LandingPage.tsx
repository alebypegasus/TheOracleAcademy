    ...
    <div className="w-full min-h-screen bg-[#080510] text-slate-200 overflow-y-auto overflow-x-hidden relative">
        ...
        {showLogin && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/75 backdrop-blur-md p-4 overflow-y-auto">
            <motion.div 
              ...
              className={`w-full ${isRegistering ? 'max-w-xl' : 'max-w-sm'} glass-panel p-8 rounded-[2.5rem] border border-indigo-500/30 relative bg-[#090615]/90 shadow-[0_0_50px_rgba(99,102,241,0.2)] my-8 transition-all`}
            >
              <button 
                onClick={() => {
                  setShowLogin(false);
              ...
              {mfaStep ? (
                <div className="space-y-6">
                  ...
                  {error && (
                    <div className="text-rose-400 text-xs bg-rose-500/10 p-3.5 rounded-2xl border border-rose-500/20 leading-relaxed font-light text-center">
                      {error}
                    </div>
                  )}
                ...
                <>
                  <div className="text-center mb-6">
                    ...
                    <h2 className="text-2xl md:text-3xl font-serif text-slate-100 tracking-wider">
                      {isRegistering ? `Registro Iniciático (${registerStep}/3)` : 'Acesse o Santuário'}
                    </h2>
                    <p className="text-xs text-indigo-300 mt-1 uppercase tracking-widest font-mono">
                      {isRegistering 
                        ? (registerStep === 1 ? 'Suas Credenciais Místicas' : registerStep === 2 ? 'Sua Sintonía com o Éter' : 'Sua Energia e Plano Celestial') 
                        : 'Entre para consultar os oráculos e seções de estudo'}
                    </p>
                  ...
                  {error && (
                    <div className="mb-6 text-rose-400 text-xs bg-rose-500/10 p-3.5 rounded-2xl border border-rose-500/20 leading-relaxed font-light">
                      {error}
                    </div>
                  )}

                  {/* TAB SELECTOR FOR EMAIL VS PHONE LOGIN */}
                  {!isRegistering && (
                    <div className="flex p-1 bg-black/40 border border-white/5 rounded-2xl mb-6">
                      <button
                        type="button"
                        onClick={() => { setAuthMethod('email'); setError(''); }}
                        className={`flex-1 py-2 text-xs font-bold uppercase tracking-wider rounded-xl transition-all ${authMethod === 'email' ? 'bg-indigo-600/20 border border-indigo-500/30 text-indigo-300' : 'text-slate-400 hover:text-slate-200'}`}
                      >
                        E-mail
                      </button>
                      <button
                        ...
                        className={`flex-1 py-2 text-xs font-bold uppercase tracking-wider rounded-xl transition-all ${authMethod === 'phone' ? 'bg-indigo-600/20 border border-indigo-500/30 text-indigo-300' : 'text-slate-400 hover:text-slate-200'}`}
                      >
                        Celular / SMS
                      </button>
                    </div>
                            ...
                            <div>
                              ...
                              <div className="relative">
                                ...
                                <input 
                                  ...
                                  className="w-full bg-black/40 border border-white/10 rounded-2xl py-3.5 pl-11 pr-4 text-sm text-slate-200 outline-none focus:border-indigo-500/60 focus:bg-black/60 transition-all font-light"
                            ...
                            <div>
                              ...
                              <div className="relative">
                                ...
                                <input 
                                  ...
                                  className="w-full bg-black/40 border border-white/10 rounded-2xl py-3.5 pl-11 pr-11 text-sm text-slate-200 outline-none focus:border-indigo-500/60 focus:bg-black/60 transition-all"
                                  placeholder="•••••"
                                />
                                <button
                                  type="button"
                              ...
                              <div>
                                ...
                                <div className="relative">
                                  ...
                                  <input 
                                    ...
                                    className="w-full bg-black/40 border border-white/10 rounded-2xl py-3.5 pl-11 pr-4 text-sm text-slate-200 outline-none focus:border-indigo-500/60 focus:bg-black/60 transition-all font-light font-mono"
                                    placeholder="+55 (11) 99999-9999"
                                  />
                                </div>
                                <p className="text-[10px] text-slate-500 mt-2 font-mono italic">
                                  Formatos: +5511999999999 ou ddd + número.
                                </p>
                              </div>
                            ) : (
                              <div>
                                ...
                                <div className="relative">
                                  <Key className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-indigo-400 text-amber-500" />
                                  <input 
                                    type="text" 
                                    required
                                    maxLength={6}
                                    ...
                                    className="w-full bg-black/40 border border-white/10 rounded-2xl py-3.5 pl-11 pr-4 text-center text-md tracking-[0.2em] font-mono text-amber-300 outline-none focus:border-indigo-500/60 focus:bg-black/60 transition-all"
                                    placeholder="000000"
                                  />
                                </div>
                                <button
                                  ...
                                  className="text-[10px] text-indigo-400 hover:text-indigo-300 mt-2 block underline"
                                >
                                  Alterar Telefone
                                </button>
                              </div>
                  ...
                  <div>
                    ...
                    {registerStep === 2 && (
                      ...
                      >
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <input 
                              ...
                              className="w-full bg-black/40 border border-white/10 rounded-2xl py-3 px-4 text-sm text-slate-200 outline-none focus:border-amber-500/50 focus:bg-black/60 transition-all"
                              placeholder="ex: serena_moon"
                            />
                          </div>
                          ...
                          <div>
                            ...
                            <select 
                              ...
                              className="w-full bg-black/40 border border-white/10 rounded-2xl py-3 px-4 text-xs text-slate-200 outline-none focus:border-amber-500/50 focus:bg-black/60 transition-all h-[46px]"
                            >
                              <option value="Santuário Sagrado">Santuário Sagrado (Astral)</option>
                              <option value="Oráculo do Éter">Oráculo do Éter (São Paulo - BR)</option>
                              <option value="Templo das Plêiades">Templo das Plêiades (Tecnologia Mística)</option>
                              <option value="Santuário Urânia">Santuário Urânia (Rio de Janeiro - BR)</option>
                            </select>
                          </div>
                        </div>

                        <div>
                          <label className="text-xs text-slate-400 uppercase tracking-widest mb-1.5 block font-bold">Seu Título / Afinidade Oracular</label>
                          <div className="grid grid-cols-2 gap-2">
                            {[
                              { id: 'Busca-Caminhos', t: 'Busca-Caminhos', desc: 'Explorador Geral' },
                              { id: 'Sacerdote Solstício', t: 'Sacerdote / Sacerdotisa', desc: 'Alta Magia Lunar' },
                              { id: 'Alquimista Mental', t: 'Alquimista Mental', desc: 'Ocultismo Hermético' },
                              { id: 'Cartomante Prático', t: 'Cartomante', desc: 'Tarot e Lenormand' }
                            ].map(item => (
                              <button
                                key={item.id}
                                type="button"
                                onClick={() => setAuthorTitle(item.id)}
                                className={`p-2.5 rounded-xl text-left border ${authorTitle === item.id ? 'border-amber-500 bg-amber-500/10 text-amber-300' : 'border-white/5 bg-white/[0.02] text-slate-400'} transition-all`}
                              >
                                <div className="text-xs font-bold leading-tight truncate">{item.t}</div>
                                <div className="text-[9px] text-slate-500 font-light truncate">{item.desc}</div>
                              </button>
                            ))}
                          </div>
                        </div>
                        ...
                        <div>
                          <label className="text-xs text-slate-400 uppercase tracking-widest mb-1.5 block font-bold">Biografia Mística (Sobre Mim)</label>
                          <textarea 
                            value={description}
                            onChange={(e) => setDescription(e.target.value)}
                            maxLength={200}
                            className="w-full bg-black/40 border border-white/10 rounded-2xl py-2.5 px-4 text-xs text-slate-200 outline-none focus:border-amber-500/50 focus:bg-black/60 transition-all h-20 resize-none font-light"
                            placeholder="Conte um pouco sobre suas afinidades espirituais..."
                          />
                        </div>
                      </motion.div>
                    )}

                    {/* STEP 3: Profile Energy, Predefined Avatars & Plans options */}
                    {registerStep === 3 && (
                      <motion.div 
                        initial={{ opacity: 0, x: 20 }} 
                        animate={{ opacity: 1, x: 0 }} 
                        className="space-y-4"
                      >
                        {/* Choice of Avatar Preset card list */}
                        <div>
                          <label className="text-xs text-slate-400 uppercase tracking-widest mb-1.5 block font-bold text-center">Incorpore seu Elemento (Avatar de Partida)</label>
                          <div className="grid grid-cols-4 gap-3 py-1">
                            {avatarPresets.map((pr, index) => (
                              <button
                                key={index}
                                type="button"
                                onClick={() => setAvatar(pr.url)}
                                className={`relative group w-full aspect-square rounded-full overflow-hidden border-2 transition-all ${avatar === pr.url ? 'border-amber-400 scale-105 shadow-[0_0_12px_rgba(245,158,11,0.55)]' : 'border-white/15 hover:border-indigo-400'}`}
                              >
                                <img src={pr.url} alt={pr.label} className="w-full h-full object-cover" />
                                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all text-[8px] font-bold text-amber-300 uppercase tracking-wider">{pr.label}</div>
                              </button>
                            ))}
                          </div>
                        </div>

                        {/* Social profiles and values */}
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <input 
                              ...
                              className="w-full bg-black/40 border border-white/10 rounded-xl py-2 px-3 text-xs text-slate-200 outline-none focus:border-amber-500/40"
                ...
                <div className="pt-2">
                  {!isRegistering ? (
                    authMethod === 'email' ? (
                      <button type="submit" className="w-full py-4 bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-500 hover:to-indigo-400 text-white rounded-2xl text-sm font-bold tracking-widest uppercase transition-all shadow-xl shadow-indigo-500/20 flex items-center justify-center gap-2">
                      ...
                      <button 
                        ...
                        className="w-full py-4 bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-500 hover:to-amber-400 text-slate-950 rounded-2xl text-sm font-bold tracking-widest uppercase transition-all shadow-xl shadow-amber-500/20 flex items-center justify-center gap-2"
                      >
                        {phoneVerifying ? (
                          <span className="animate-pulse">CONFIRMANDO SINTONIA...</span>
                        ) : phoneStep === 'input' ? (
                          <>SOLICITAR CÓDIGO SMS <ArrowRight className="w-4 h-4" /></>
                        ) : (
                          <>AUTENTICAR CELULAR <ShieldCheck className="w-4 h-4" /></>
                        )}
                      </button>
                    )
                  ) : (
                    <div className="flex gap-4">
                      {registerStep > 1 && (
                        <button 
                          ...
                          className="px-5 py-3.5 bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 rounded-2xl text-xs font-bold uppercase tracking-wider transition-colors"
                        >
                          Voltar
                        </button>
                      )}
                      <button 
                        type="submit" 
                        className="flex-1 py-3.5 bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-500 hover:to-amber-400 text-slate-950 rounded-2xl text-xs font-black tracking-widest uppercase transition-all shadow-lg shadow-amber-500/10 flex items-center justify-center gap-2"
                      >
              ...
              {!isRegistering && !mfaStep && (
                <div className="mt-6 space-y-4">
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-white/5 hover:bg-white/10 border border-white/10 text-slate-200 rounded-2xl text-xs font-bold tracking-wider uppercase transition-colors flex items-center justify-center gap-3"
                  >
              ...
              {/* Recaptcha Container Anchor for Firebase Auth client */}
              ...
              <div className="mt-8 text-center pt-4 border-t border-white/5">
                ...
                >
                  {isRegistering ? 'Já possui chaves de acesso? Entre' : 'Ainda não é iniciado? Solicite cadastro místico'}
                </button>
              </div>
            </motion.div>
          </div>
      ...
      <section className="relative min-h-[90vh] flex flex-col items-center justify-center pt-24 pb-16 px-4 text-center overflow-hidden">
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 w-full max-w-4xl bg-black/50 border border-white/5 p-6 rounded-[2.5rem] backdrop-blur-md">
            {[
              { id: "stat-1", icon: <Users className="w-5 h-5 text-indigo-400" />, num: "14.840+", text: "Almas Matriculadas" },
              { id: "stat-2", icon: <GraduationCap className="w-5 h-5 text-amber-400" />, num: "4.212+", text: "Diplomas Concluídos" },
              { id: "stat-3", icon: <Sparkles className="w-5 h-5 text-emerald-400" />, num: "98.9%", text: "Alinhamento Divino" },
              { id: "stat-4", icon: <Compass className="w-5 h-5 text-cyan-400" />, num: "285.000+", text: "Sintonias de Oráculos" }
            ].map(st => (
              <div key={st.id} className="text-center p-3">
                <div className="mx-auto w-10 h-10 rounded-full bg-white/5 flex items-center justify-center mb-2">{st.icon}</div>
                <div className="text-2xl font-serif text-slate-100 font-bold tracking-tight">{st.num}</div>
                <div className="text-xs text-slate-500 mt-1 uppercase tracking-wider font-mono">{st.text}</div>
              </div>
            ))}
          </div>
        </motion.div>
            ...
            {zodiacSigns.map(sign => {
              ...
              return (
                <button
                  ...
                  className={`p-4 rounded-2xl border text-center transition-all duration-300 relative group overflow-hidden ${
                    isSelected 
                      ? 'border-amber-400 bg-amber-400/10 scale-105 shadow-[0_0_20px_rgba(245,158,11,0.2)]'
                      : 'border-white/5 bg-black/40 hover:border-slate-700 hover:bg-slate-900/50'
                  }`}
                >

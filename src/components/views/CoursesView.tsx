                   ...
                   <div className="w-full h-[600px] bg-black/40 rounded-[2rem] border border-indigo-500/30 overflow-hidden relative shadow-2xl flex flex-col group">
                        ...
                        {aiChat.map((msg, idx) => (
                           <div key={idx} className={`flex w-full ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                              <div className={`flex gap-4 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                                 <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 border ${msg.role === 'assistant' ? 'bg-indigo-500/20 border-indigo-500/30 text-indigo-400' : 'bg-emerald-500/20 border-emerald-500/30 text-emerald-400'}`}>
                                    {msg.role === 'assistant' ? <Bot className="w-5 h-5" /> : <div className="font-bold text-xs">V</div>}
                                 </div>
                                 <div className={`p-4 rounded-2xl text-sm leading-relaxed ${msg.role === 'user' ? 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-50' : 'bg-indigo-500/10 border border-indigo-500/20 text-indigo-50'}`}>
                                    {msg.content}
                                 </div>
                              </div>
                           </div>
                     ...
                     <div className="p-4 bg-[#0a0a0a]/80 border-t border-white/5 relative z-10">
                        ...
                        <p className="text-[10px] text-center text-slate-500 mt-3 uppercase tracking-widest">
                           A avaliação moldará os próximos desafios desbloqueados
                        </p>
                     </div>
                  </div>
                ) : (
                   ...
                   <div className="glass-panel p-8 rounded-3xl border border-white/5 bg-white/[0.01] md:col-span-2 flex flex-col items-center justify-center text-center">
                      ...
                      <div className="flex gap-2">
                         {[1, 2, 3, 4, 5].map((star) => (
                           ...
                           >
                             <Tooltip content="O sistema adaptará a próxima lição baseando-se em sua avaliação">
         ...
         <div className="flex-1 w-full max-w-5xl mx-auto py-16 px-4 md:px-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               {INNER_PATH.map((step, index) => {
                  ...
                  let IconToRender = BookOpen;
                  let tooltipText = "Módulo";
                  if (isCompleted) { IconToRender = Check; tooltipText = "Concluído" }
                  else if (isLocked) { IconToRender = Lock; tooltipText = "Bloqueado" }
                  else if (step.type === 'desafio') { IconToRender = Calendar; tooltipText = "Desafio" }
                  else if (step.type === 'prova') { IconToRender = Bot; tooltipText = "Avaliação via Chat (IA)" }
                  ...
                  return (
                     ...
                     >
                        {isCurrent && (
                          <motion.div layoutId="active-step-border" className={`absolute top-0 left-0 w-full h-1 ${selectedNode.tailwindColor}`} />
                        )}
         ...
         <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div className="flex items-center gap-3">
               ...
               <div className="px-4 py-2 rounded-xl theme-bg-primary-soft border theme-border-primary-soft flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-slate-200">12 / 48</span>
               </div>
            </div>
         </div>
      </div>
